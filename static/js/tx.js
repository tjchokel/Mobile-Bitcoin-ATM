...
coinpunk.controllers.Tx.prototype.scanQR = function(event) {
  ...
  var img = new QrImageDecoder({
      src: src,
      success: qrReadSuccess,
      error: qrReadError
  });

};

function qrReadError(result) {
    return $("#errors").removeClass('hidden').text('Could not process the QR code, the image may be blurry. Please try again.');
}

...
def custom_confirm(body_template, merchant):
    '''
...
def send_nag_email(body_template, merchant, subject, context_dict={}):
...
class Command(BaseCommand):
    ...
    def handle(self, verbose, *args, **kwargs):
        ...
        dp('%s merchant signups to try...' % merchants.count())
        for merchant in merchants:
            ...
            if not api_cred:
                ...
                context_dict = {'store_name': merchant.business_name}
            ...
            if not merchant.address_1:
                ...
                subject = "Where is %s?" % (merchant.business_name)
                send_nag_email(
                        subject=subject,
                        body_template=body_template,
                        merchant=merchant,
            ...
            has_phone = bool(merchant.phone_num)
            has_logo = merchant.has_doc_obj()
            has_website = merchant.has_website()

            context_dict['profile_uri'] = merchant.get_profile_uri()

            if not has_phone:
                context_dict['add_phone_uri'] = reverse('merchant_profile')

                body_template = 'drip/no_phone.html'
                subject = "What is the Phone Number at %s?" % merchant.business_name
                send_nag_email(
                        subject=subject,
                        body_template=body_template,
                        merchant=merchant,
            ...
            if not has_logo:
                ...
                subject = "Does %s Have a Logo?" % merchant.business_name
                send_nag_email(
                        subject=subject,
                        body_template=body_template,
                        merchant=merchant,
            ...
            if not has_website:
                ...
                subject = "Does %s Have a Website?" % merchant.business_name
                send_nag_email(
                        subject=subject,
                        body_template=body_template,
                        merchant=merchant,
            ...
            # None applicable
            msg = "Not contacting %s because they're all good!" % merchant
            dp(msg)
